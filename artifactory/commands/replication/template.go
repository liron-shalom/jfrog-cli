...
func (rtc *ReplicationTemplateCommand) Run() (err error) {
	replicationTemplateQuestionnaire := &utils.InteractiveQuestionnaire{
		MandatoryQuestionsKeys: []string{TemplateType, JobType, RepoKey},
...
var questionMap = map[string]utils.QuestionInfo{
	...
	TemplateType: {
		Options: []prompt.Suggest{
			...
			{Text: Update, Description: "Template for updating a replication job"},
		},
...
func jobTypeCallback(iq *utils.InteractiveQuestionnaire, jobType string) (string, error) {
	...
	case Update:
		if jobType == Pull {
			iq.OptionalKeysSuggests = getAllPossibleOptionalRepoConfKeys(CronExp)
		} else {
			iq.OptionalKeysSuggests = append(iq.OptionalKeysSuggests, getAllPossibleOptionalRepoConfKeys(Username, Password, URL, CronExp)...)
		}
	}
	return "", nil
}
